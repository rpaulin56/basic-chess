 ...
 *
 *   npm run i18n:export -- --out traduzione-fr.csv
 *
...
/**
 * Da che parte del programma viene una chiave, riconosciuta dal prefisso.
 *
 ...
 * pannello.
 */
const CONTEXTS: readonly [RegExp, string][] = [
  [/^tutor/, 'Pannello della Nonna quando segnala un errore. E’ lei che parla, in prima persona.'],
  [/^(cat|pos|piece|equivalent|mate|cross|head)/, 'Frasi con cui la Nonna spiega un errore. Voce della Nonna.'],
  [/^orient/, '"E adesso?": la Nonna descrive la posizione quando le mosse buone sono molte.'],
  [/^hint/, '"E adesso?": quante e quali mosse tengono la posizione.'],
  [/^why/, 'Analisi di fine partita. Voce della Nonna.'],
  [/^recap/, 'Riepilogo degli errori della partita, nella colonna di destra.'],
  [/^(resign|draw|offer|outcome)/, 'Abbandono e offerta di patta, con il giudizio della Nonna.'],
  [/^eg/, 'Nomi dei finali teorici e schede di studio.'],
  [/^opponentHelp/, 'Finestra "?" che spiega come scegliere il livello e la distrazione.'],
  [/^(level|distraction|playAs)/, 'Scelta dell’avversaria, sotto la scacchiera.'],
  [/^credits?/, 'Pannello dei crediti e delle licenze.'],
  [/^(show|settings|language)/, 'Finestra delle impostazioni.'],
  [/^(import|export|position|copied|overwrite)/, 'Menu per far entrare e uscire posizioni e partite.'],
  [/^(rewind|turn|thinking|checkmate|stalemate|draw|newGame|recover|first|last|previous|next|flip|takeBack|moves|noMoves|opening)/, 'Comandi e riga di stato sotto la scacchiera. Spazio stretto.'],
  [/^(engine|analysing|eval)/, 'Riga della valutazione sotto la scacchiera. Spazio stretto.'],
  [/^app|^tagline/, 'Intestazione della pagina.'],
];

function contextOf(key: string): string {
  for (const [pattern, description] of CONTEXTS) {
    if (pattern.test(key)) return description;
  }
  return 'Varie.';
}
...
for (const key of Object.keys(it) as (keyof typeof it)[]) {
  ...
  const notes: string[] = [];
  if (marks) notes.push('Ricopia i segnaposto identici.');
  ...
  if (italian.length <= 25) notes.push('Spazio stretto: tieniti corta come l’originale.');
